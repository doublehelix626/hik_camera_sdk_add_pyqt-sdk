...
1. 当前项目更适合作为开发示例和实验工程使用。
2. 不建议将真实设备 IP、用户名和密码直接提交到公开仓库。
3. 建议后续将设备配置改为：
   - `config.yaml`
   - `.env`
   - 或单独的本地配置文件
4. 建议添加 `.gitignore` 文件，以避免将日志、IDE 配置文件以及 SDK 运行缓存一并提交。
...
用于探索海康 PTZ 摄像机在桌面 GUI 环境下的接入、显示与控制。

---

## License

如果你计划将本项目开源，建议补充许可证，例如：

- MIT
- Apache-2.0
- GPL-3.0

如果你暂时还没有决定，也可以先保留这一部分，后续再补充。
